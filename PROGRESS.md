...
## Phase 8 — Frozen food module, polish, SEO, analytics ✅ done

- **Frozen food module** (`/order/frozen`, spec section 5): the real flow, replacing the phase-4 stub. Public catalog of active `category: "frozen"` items with published $/lb or $/L pricing, per-product quantity pickers, live subtotal + estimated-total-with-tax in a sticky bar, a pickup date/time (no delivery — locked Q6 decision) gated by the *slowest* lead time across the cart, and a contact form pre-filled for signed-in customers. Everything is re-validated server-side against the live catalog (price, minimum quantity, lead time) — never trusts the client. Routes into the same `orders` pipeline with `orderType: "frozen_food"`.
- **A real data-modeling gap, handled deliberately rather than migrated around**: `orders` has several NOT NULL columns (`guestCount`, `pricePerPersonCents`, `budgetMode`, `budgetAmountCents`) that don't semantically apply to a frozen pickup order. Rather than a schema migration this late with zero production data at stake either way, used honest, non-misleading sentinels (`guestCount: 0`, `pricePerPersonCents: 0`, `budgetMode: "total"` with `budgetAmountCents` set to the real grand total) and hid the "0 guests" artifact from every admin view (orders list, dashboard) via an `orderType` check. Documented here rather than silently.
- **Admin side**: the existing band-centric review workspace assumes a `bandCode` everywhere (margin panel, fee overrides, quote versioning) and would throw on a null-band order. Added a branch in `/admin/orders/[id]` that routes frozen orders to a new, much simpler `FrozenOrderReview` — order summary, items, notes, and a status dropdown. No quote-negotiation UI, since frozen prices are published and fixed, not quoted.
- **Closed a real gap found while building that view**: there was no way for an admin to move *any* order (banded or frozen) to `scheduled` or `completed` — status changes only ever happened automatically via customer approval or "send revised quote". Added a shared `OrderStatusSelect` control (all 9 statuses) to both review views.
- **Design polish** (spec section 8, untouched since the phase-1 scaffold): added the warm palette ("one warm accent, deep neutral, off-white") as real `oklch` tokens in `globals.css`, and a serif heading font (Fraunces) wired through the theme's existing (but previously-disconnected) `--font-heading` token.
  - **Found a real pre-existing bug while doing this**: `--font-sans` was defined as `var(--font-sans)` — a circular self-reference — so the site had been silently falling back to the browser's default sans-serif this whole time instead of actually rendering Geist Sans, since phase 1. Fixed by pointing it at the loaded `--font-geist-sans` variable.
  - **Found a second real bug via an actual screenshot, not just a green build**: every `<Button render={<Link .../>}>` pattern (used since phase 1 for button-styled links) was tripping a Base UI accessibility warning — `nativeButton` defaults to `true`, expecting the rendered element to really be a `<button>`, which a `<Link>` (an `<a>`) isn't. Fixed all 6 occurrences with `nativeButton={false}`. Verified via screenshot that Next's dev-overlay issue count actually dropped to zero afterward.
- **SEO**: `robots.ts` and `sitemap.ts` (Next's metadata-file convention), `noindex` on every private surface (`/admin/*`, `/account/*`, `/login/*`, `/quote/[id]`), Open Graph + Twitter card metadata and `metadataBase` on the root layout.
- **Analytics**: `@vercel/analytics` and `@vercel/speed-insights` — zero-config, activate automatically on a Vercel deploy, no-op (and confirmed harmless in debug mode) locally.
- Verified everything with the same real-browser approach as prior phases: screenshotted the home page, wizard step 1, and admin dashboard to confirm the new palette/fonts actually render (not just compile), then ran the full frozen-food flow end to end — including a below-minimum-quantity rejection — and hand-verified the resulting order, its packing-sheet PDF, and its admin review page all show the exact same numbers.
- Hit real Puppeteer quirks while testing (not app bugs, but worth noting for future sessions): native `<input type="date">`/`<input type="time">` elements don't respond to Puppeteer's `.fill()` in this environment's pinned Chrome build — silently leaves them empty, which then correctly fails client validation. Direct DOM value-setting (`Object.getOwnPropertyDescriptor` + dispatch `input`) is the reliable workaround, alongside the already-known direct-`.click()`-over-text-locator pattern from phases 6-7.
